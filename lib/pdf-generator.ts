...
export async function generateFireAlarmPDF(data: FireAlarmFormData): Promise<Uint8Array> {
  ...
  leftCol.forEach((label, index) => {
    ...
    const value = data.controlPanelStatus[key]
  ...
  addText(`System Put in Test At: ${data.putSystemInTestAt}`, leftMargin + 10, yPosition, { size: 10, bold: true })
  yPosition -= lineHeight
  ...
  data.equipmentTested.forEach((equipment) => {
    if (equipment.equipmentLabel) {
      ...
      addText(equipment.totalNumberTested.toString(), currentX + 5, yPosition - 12, { size: 9 })
      currentX += equipColWidths[1]
      ...
      addText(equipment.functionOK, currentX + 5, yPosition - 12, { size: 9 })
  ...
  functionalTestLabels.forEach((label, index) => {
    ...
    const value = data.functionalTest[key]
    addText(`${label}:`, leftMargin + 10, yPosition, { size: 10 })
  ...
  const powerSupplyInfo = [
    [`Primary Power:`, `${data.primaryPower} Amps`],
    [`Nominal Voltage:`, `${data.nominalVoltage} Volts`],
    [`Overcurrent Protection:`, data.overcurrentProtection],
    [`Panel Breaker Location:`, data.panelBreakerLocation],
    [`Battery Test Reading:`, `${data.batteryTestReadingVolts}V, ${data.batteryTestReadingAmps}A`],
    [`Storage Battery:`, `${data.storageBatteryAmpHour} Amp Hours`],
    [`Hours System Must Operate:`, `${data.hoursSystemMustOperate} hours`],
  ...
  addText(`Name: ${data.testVerificationCEC.name}`, leftMargin + 25 + signatureColWidth, yPosition - 35, { size: 10 })
  addText(`Title: ${data.testVerificationCEC.title}`, leftMargin + 25 + signatureColWidth, yPosition - 50, { size: 10 })
  addText(`Date: ${data.testVerificationCEC.date}`, leftMargin + 25 + signatureColWidth, yPosition - 65, { size: 10 })
